...
uint64_t Engine::get_move(const Board& board)
{
    ...
    int best_score = (board.turn == 1) ? std::numeric_limits<int>::min() : std::numeric_limits<int>::max();
    ...
    for (int i = 0; i < 9; ++i)
    {
        ...
        Board next = (board.turn == 1)
            ? Board(board.player1 | move, board.player2, 2)
...
int Engine::evaluate_board(const Board& board)
{
    for (auto mask : WIN_MASKS)
    {
        if ((board.player1 & mask) == mask)
            return 10; // player1 wins
        if ((board.player2 & mask) == mask)
            return -10; // player2 wins
    }
